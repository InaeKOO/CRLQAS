...
from environment.environment_tc_4q_median_restart_experiment import CircuitEnv
import agents
...
if __name__ == '__main__':
    ...
    environment = CircuitEnv(conf, device=device, args = args)
    ...
    if conf['agent']['init_net']:  
        ...
        print(PATH)
        agent.policy_net.load_state_dict(torch.load(PATH + f"_model.pth",map_location=f"cuda:{args.gpu_id}"))
        agent.target_net.load_state_dict(torch.load(PATH + f"_model.pth",map_location=f"cuda:{args.gpu_id}"))
        agent.optim.load_state_dict(torch.load(PATH + f"_optim.pth",map_location=f"cuda:{args.gpu_id}"))
        agent.policy_net.eval()
        agent.target_net.eval()

        replay_buffer_load = torch.load(f"{PATH}_replay_buffer.pth",map_location=f"cuda:{args.gpu_id}")
