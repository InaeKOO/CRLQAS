...
from environment.environment_tc_3q_max import CircuitEnv
import agents
...
# from ptm_functions import rho0_4_ptm, H_4_ptm, load_ham_dict, PTM_representation, PTM_matrix_representation
...
def one_episode(episode_no, env, agent, episodes):
    ...
    for itr in range(env.num_layers + 1):
        ...
        if done:
            ...
            # print(f"Episode time {time.time() - t}")
            if episode_no % 2 == 0:
        ...
        if len(agent.memory) > conf['agent']['batch_size']:
            ...
            # print(loss)
            # exit()
    # agent.saver.stats_file['train'][episode_no]['time'].append(tctr)
    # print(f"episode w.o training takes {tctr}")
...
def train(agent, env, episodes, seed, output_path, threshold):
    ...
    for e in range(episodes):
        ...
        # print(f"curriculum type is {env.curriculum_type }")
        ...
        if env.curriculum_type == 'VanillaCurriculum':
           ...
           if e % 100 == 0:
               final_rwd, iteration = agent_test(env, agent, e, seed, output_path, threshold)
        #else:
        #   raise ValueError(f'Curriculum type not configured properly!')
        # TODO: TESTING IS OFF FOR SPEED
        # final_rwd, iteration = agent_test(env, agent, e, seed, output_path,threshold)
        # ep_time = time.perf_counter() - t
        # agent.saver.stats_file['train'][e]['time'].append(ep_time)
        # print(f"Time of episode is {ep_time}")
