...
def agent_test(env, agent, episode_no, seed, output_path, threshold):
    ...
    for itr in range(env.num_layers + 1):
        ...
        if done:
            ...
            errors_current = [val['errors'][-1] for val in agent.saver.stats_file['test'].values()
                                   if val['done_threshold'] == env.done_threshold]
            if len(errors_current) > 0 and min(errors_current) > env.error:
...
if __name__ == '__main__':
    ...
    conf = get_config(args.experiment_name, f'uscl.cfg', path=path_config)
    ...
    agent = DeepQNstep.DQN_Nstep(conf,environment.action_size,environment.state_size,device)
